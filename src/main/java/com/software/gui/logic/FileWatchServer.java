...
public class FileWatchServer extends Thread {
    ...
    public void run() {
        try {
            logger.info("folder watching start!");
            new WatchDir(new File(cache.getDiskRealPath()), false, new FileActionCallback() {
                @Override
                public void delete(File file) {
                    Platform.runLater(new Runnable() {
                        @Override
                        public void run() {
                            cache.autoDeleteFromCache(file);
                        }
                    });
                ...
                public void create(File file) {
                    if(file.getName().endsWith(".jar") || file.getName().endsWith(".litemod")) {
                        cache.autoAddToCache(file);
                        logger.info("add new meaningful file:"+file.getName());
                    } else
                        cache.addToIgnore(file.getAbsolutePath());
                }
        ...
        logger.info("watch finished at " + path);
    }
};
...
class WatchDir {
     ...
     * @param file
     *            文件目录，不可以是文件
     * @param subDir if true,all subDir will be register to monitor
     * @throws Exception
     */
